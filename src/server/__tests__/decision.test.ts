...
async function del(path: string, body: unknown) {
  return jsonFetch(path, { method: 'DELETE', body: JSON.stringify(body) })
}
...
describe('POST /api/events/:id/decision', () => {
  ...
  it('D2: 別 organizer で POST → 403', async () => {
    ...
    const res = await post(`/api/events/${eventId}/decision`, {
      candidateId,
      actorDiscordId: OTHER_ORGANIZER_ID,
    })
  ...
  it('D5: 再 POST で別 candidate → 200 + 同じ icsUid + icsSequence=1', async () => {
    ...
    // 最初の確定
    const res1 = await post(`/api/events/${eventId}/decision`, {
    ...
    // 再確定（別 candidate）
    const res2 = await post(`/api/events/${eventId}/decision`, {
...
describe('DELETE /api/events/:id/decision', () => {
  it('D6: DELETE 正常 → 200 + cancelledAt 設定 + status=open + icsSequence+1', async () => {
    ...
    // 取り消し
    const res = await del(`/api/events/${eventId}/decision`, {
      actorDiscordId: ORGANIZER_ID,
    })
...
describe('Decision シーケンス管理', () => {
  it('D9: DELETE 後再 POST → 200 + 同じ icsUid 保持 + cancelledAt=null + icsSequence さらに +1', async () => {
    ...
    // 取り消し (icsSeq=1)
    const res2 = await del(`/api/events/${eventId}/decision`, {
      actorDiscordId: ORGANIZER_ID,
    })
    ...
    // 再確定 (icsSeq=2)
    const res3 = await post(`/api/events/${eventId}/decision`, {
      candidateId: candidateId2,
      actorDiscordId: ORGANIZER_ID,
    })
  ...
  it('D10: AuditLog に decision.create と decision.cancel が記録される', async () => {
    ...
    await del(`/api/events/${eventId}/decision`, {
      actorDiscordId: ORGANIZER_ID,
    })
